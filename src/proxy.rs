...
impl Proxy{
    ...
    pub async fn check() {
        ...
        let thread_count: Arc<Mutex<i32>> = Arc::new(Mutex::new(0));
        ...
        for current_proxy in proxy_list { 
            ...
            let thread_count_local = Arc::clone(&thread_count);
            pool.execute(move || {
                println!("{}",thread_count_local.lock().unwrap());
                *thread_count_local.lock().unwrap() += 1;
                ...
                match client_request.send()  {
                    Ok(response) => {
                        if response.status() == StatusCode::from_u16(200).unwrap() {
                            ...
                            checked_proxy_list_local.lock().unwrap().push(current_proxy)
                        }
                    }
                    Err(..) => {}
                }
